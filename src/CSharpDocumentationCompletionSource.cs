...
namespace NerdyDuck.CSharpDocumentationCompletion2017
{
	class CSharpDocumentationCompletionSource : IAsyncCompletionSource
	{
		...
		public bool TryGetApplicableToSpan(char typedChar, SnapshotPoint triggerLocation, out SnapshotSpan applicableToSpan, CancellationToken cancellationToken)
		{
			applicableToSpan = _navigator.GetExtentOfWord(triggerLocation).Span;
			...
			{
				if (typedChar == '<') //|| (trigger.Character == '\0' && (trigger.Reason == CompletionTriggerReason.Invoke || trigger.Reason == CompletionTriggerReason.InvokeAndCommitIfUnique))) 
				{
					return true;
				}
				else
		...
		public Task<object> GetDescriptionAsync(CompletionItem item, CancellationToken token) => Task.Run(() =>
		{
